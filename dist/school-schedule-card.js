var _="school-schedule-card",v={title:"Stundenplan",show_date:!0,show_footer_hints:!0,view:"week",tomorrow_after:"16:00",courses:void 0,hide_subjects:void 0,dense:!1},A={Mat:{bg:"#2196f3",fg:"#fff"},Deu:{bg:"#673ab7",fg:"#fff"},Eng:{bg:"#4caf50",fg:"#fff"},Bio:{bg:"#8bc34a",fg:"#fff"},Che:{bg:"#fb8c00",fg:"#fff"},Phy:{bg:"#03a9f4",fg:"#fff"},Ges:{bg:"#e91e63",fg:"#fff"},Geo:{bg:"#2e7d32",fg:"#fff"},Mus:{bg:"#fdd835",fg:"#212121"},Kun:{bg:"#7e57c2",fg:"#fff"},Spo:{bg:"#26a69a",fg:"#fff"},Rel:{bg:"#9e9e9e",fg:"#fff"},Eth:{bg:"#9e9e9e",fg:"#fff"},eth3:{bg:"#9e9e9e",fg:"#fff"},Inf:{bg:"#3f51b5",fg:"#fff"},Spa:{bg:"#ff5722",fg:"#fff"},Mlk:{bg:"#ff5722",fg:"#fff"}};function M(t){return t&&typeof t=="object"&&!Array.isArray(t)}function E(t){try{return JSON.parse(t)}catch{return null}}function N(t){if(!Array.isArray(t))return[];let n=e=>{if(M(e)){let c=e._??e["#text"]??"",a=!!(e.$&&Object.keys(e.$).length),d=Object.keys(e).some(f=>f.startsWith("@")&&e[f]!=null&&String(e[f]).length>0),l=a||d;return{text:String(c||""),changed:l}}return{text:String(e??""),changed:!1}};return t.map(e=>{let c=e.subject??e.fa??e.subj,a=e.teacher??e.le??e.t,d=e.room??e.ra??e.r??"",l=n(c),f=n(a),u=n(d),s=e.if??e.info??e.infoText??"",r=e.ku??e.isCourse,i=typeof r=="boolean"?r:String(r||"").toLowerCase()==="true",o=String(s||""),h=o.toLowerCase().includes("vertret")||!!l.changed||!!f.changed;return{period:e.period??e.st??e.hour??"",time:e.time??e.beginn??e.start??"",end:e.end??e.ende??e.until??"",subject:l.text,subjectChanged:l.changed,teacher:f.text,teacherChanged:f.changed,room:u.text,roomChanged:u.changed,info:o,isCourse:i,isSubstitution:h}})}function g(t){return new Date(t)}function S(t){if(!(t instanceof Date)||isNaN(t.getTime()))return"";let n=t.getFullYear(),e=(t.getMonth()+1).toString().padStart(2,"0"),c=t.getDate().toString().padStart(2,"0");return`${n}-${e}-${c}`}function R(t){let n=t.getDay()||7,e=new Date(t);return e.setHours(0,0,0,0),e.setDate(e.getDate()-(n-1)),e}function D(t){if(!t)return[];let n=e=>{let c=String(e||"").trim();return c.length?c.replace(/[_\-\s]/g,"").length>0:!1};return Array.isArray(t)?t.map(e=>String(e)).filter(n):typeof t=="string"?n(t)?[t]:[]:[]}function C(t){if(!t)return[];if(typeof t=="string"){let n=E(t);n&&(t=n)}if(M(t)&&Array.isArray(t.days)){let n=(t.days||[]).map(s=>({name:s.name??void 0,date:s.date??void 0,lessons:N(s.lessons??[]),hints:D(s.hints),updated_at:s.updated_at??s.updatedAt??s.last_update??void 0})),e=s=>{if(s&&s.date){let o=g(s.date);if(!isNaN(o)){let h=o.getDay();return h===0?7:h}}let r=(s&&s.name?String(s.name):"").toLowerCase();return{montag:1,mo:1,monday:1,mon:1,dienstag:2,di:2,tuesday:2,tue:2,tues:2,mittwoch:3,mi:3,wednesday:3,wed:3,donnerstag:4,do:4,thursday:4,thu:4,thur:4,thurs:4,freitag:5,fr:5,friday:5,fri:5,samstag:6,sa:6,saturday:6,sat:6,sonntag:7,so:7,sunday:7,sun:7}[r]||void 0},c=new Map;for(let s of n){let r=e(s);if(!r)continue;let i=c.get(r);if(!i){c.set(r,s);continue}let o=!!i.date,h=!!s.date;c.set(r,h&&!o?s:i)}let a=n.find(s=>s.date&&g(s.date)),d=a?g(a.date):new Date,l=R(d),f=["Montag","Dienstag","Mittwoch","Donnerstag","Freitag"],u=[];for(let s=0;s<5;s++){let r=s+1,i=c.get(r),o=new Date(l);o.setDate(l.getDate()+s);let h=S(o);i?u.push({name:i.name??f[s],date:i.date?S(g(i.date)):h,lessons:Array.isArray(i.lessons)?i.lessons:[],hints:D(i.hints),updated_at:i.updated_at??void 0}):u.push({name:f[s],date:h,lessons:[],hints:[]})}return u}return[]}function j(t){if(!t)return"";try{let n=g(t);return!n||isNaN(n.getTime())?t:n.toLocaleDateString(void 0,{year:"numeric",month:"2-digit",day:"2-digit"})}catch{return t}}function z(t){let n=(t||"").trim(),e=A[n];return e?`background:${e.bg};color:${e.fg}`:""}function H(t){let n=(t||"").trim(),e=A[n];return e?e.bg:"var(--primary-color)"}function $({dobj:t,show_date:n,show_footer_hints:e,dense:c,view:a}){let d=t.date?j(t.date):"",l=Array.isArray(t.lessons)?t.lessons:[],f=l.length?l.map(o=>{let h=!!o.isSubstitution,m=h&&o.info||"";return`
            ...
            <div class="lesson-main-content">
              ...
              <div class="lesson-content">
                ...
                <div class="lesson-details">
                  ...
                  ${o.room?`<div class="room ${o.roomChanged?"changed":""}">${o.room}</div>`:""}
                </div>
              </div>
            </div>
            ${h&&m?`<div class="substitution-info">${m}</div>`:""}
          </div>`}).join(""):"",u=D(t.hints),s=o=>{let h=(o||"").toLowerCase();return h.includes("gesperrt")||h.includes("achtung")||h.includes("warnung")?"warning":h.includes("klassenarbeit")||h.includes("pr\xFCfung")?"info":""},r=e&&u&&u.length?`
        <div class="daily-notes collapsed">
          ...
          <div class="notes-list">
            ${u.map(o=>`<div class="note-item ${s(o)}">${o}</div>`).join("")}
          </div>
            ...
            <div>
                ...
                ${t.updated_at?`<div class="updated-date" title="Letzte Aktualisierung">Stand: ${t.updated_at}</div>`:""}
            </div>
          </div>`:`<div class="header">
            <div class="header-left"><div class="day-title">${t.name||(t.date?g(t.date)?.toLocaleDateString(void 0,{weekday:"long"}):"")}</div></div>
...
`;var k=class extends HTMLElement{setConfig(n){this._config={...v,...n},this._root||(this._root=this.attachShadow({mode:"open"}),this._card=document.createElement("ha-card"),this._style=document.createElement("style"),this._container=document.createElement("div"),this._container.className="container",this._card.appendChild(this._style),this._card.appendChild(this._container),this._root.appendChild(this._card)),this._hass||(this._style.textContent=L,this._container.innerHTML=`<div class="school-schedule-card"><div class="header"><div class="day-title">${this._config.title||"Stundenplan"}</div><div class="date">Wird geladen \u2026</div></div></div>`),this._lastDataSig=void 0,this._maybeRender()}set hass(n){this._hass=n,this._maybeRender()}_getDataSignature(){let n=this._config||v,e={title:n.title,show_date:n.show_date,show_footer_hints:n.show_footer_hints,entity:n.entity?String(n.entity):void 0,has_inline_schedule:!!n.schedule,courses:Array.isArray(n.courses)?n.courses.map(a=>String(a).toLowerCase().trim()):void 0,hide_subjects:Array.isArray(n.hide_subjects)?n.hide_subjects.map(a=>String(a).toLowerCase().trim()):void 0},c=this._parseData();return JSON.stringify({cfg:e,days:c})}_maybeRender(){let n=this._getDataSignature();if(n===this._lastDataSig)return;this._lastDataSig=n;let e=this._container?this._container.querySelector(".grid"):null,c=e?e.scrollLeft:0;this._render(),this._wireNotesToggle();let a=this._container?this._container.querySelector(".grid"):null;if(a&&c)try{a.scrollLeft=c}catch{}}_parseData(){let n=this._config||{},e=d=>{if(!Array.isArray(d))return[];let l=Array.isArray(n.courses)?n.courses.map(s=>String(s).toLowerCase().trim()).filter(Boolean):null;if(!l||l.length===0)return d;let f=s=>{let r=String(s.subject||""),i=String(s.info||"");return i&&(i.trim().split(/\s+/)[0]||"")||r},u=s=>l.includes(String(s||"").toLowerCase().trim());return d.map(s=>({...s,lessons:Array.isArray(s.lessons)?s.lessons.filter(r=>!r.isCourse||u(f(r))):[]}))},c=d=>{if(!Array.isArray(d))return[];let l=Array.isArray(n.hide_subjects)?n.hide_subjects.map(s=>String(s).toLowerCase().trim()).filter(Boolean):null;if(!l||l.length===0)return d;let f=s=>{let r=String(s.subject||"");return r&&r!=="---"?r:String(s.info||"").trim().split(/\s+/)[0]||""||r},u=s=>l.includes(String(s||"").toLowerCase().trim());return d.map(s=>({...s,lessons:Array.isArray(s.lessons)?s.lessons.filter(r=>!u(f(r))):[]}))};if(n.schedule&&Array.isArray(n.schedule.days))return c(e(C(n.schedule)));let a=n.entity;if(this._hass&&a&&this._hass.states&&this._hass.states[a]){let l=this._hass.states[a].attributes||{};if(Array.isArray(l.days))return c(e(C(l)))}return[]}_render(){if(!this._root)return;let n=this._config||v,{show_date:e,show_footer_hints:c,view:a,tomorrow_after:d,dense:l}=n;a==="week"&&this._card.setAttribute("style","background: none;"),this._style.textContent=L;let f=this._parseData(),u;if((a||"week")==="day"){let r=w=>{let y=String(w||"").match(/^(\d{1,2}):(\d{2})$/),p=new Date;if(!y)return new Date(p.getFullYear(),p.getMonth(),p.getDate(),23,59,59);let x=Math.max(0,Math.min(23,parseInt(y[1],10))),T=Math.max(0,Math.min(59,parseInt(y[2],10)));return new Date(p.getFullYear(),p.getMonth(),p.getDate(),x,T,0)},i=new Date,o=r(d),h=new Date(i.getFullYear(),i.getMonth(),i.getDate()),m=(h.getDay()+6)%7,b;m===5||m===6||m===4&&i>=o?b=new Date(h.getTime()+(7-m)*864e5):i>=o?b=new Date(h.getTime()+864e5):b=h,u=((w,y)=>w.find(p=>{if(!p.date)return!1;let x=g(p.date);return!x||isNaN(x.getTime())?!1:x.getFullYear()===y.getFullYear()&&x.getMonth()===y.getMonth()&&x.getDate()===y.getDate()}))(f,b),u||(u={name:b.toLocaleDateString(void 0,{weekday:"long"}),date:S(b),lessons:[],hints:[]})}else{let r=o=>$({dobj:o,show_date:e,show_footer_hints:c,dense:l,view:a}),i=`<div class="grid">${f.map(r).join("")}</div>`;this._container.innerHTML=`${i}`;return}let s=$({dobj:u,show_date:e,show_footer_hints:c,dense:l,view:a});this._container.innerHTML=`${s}`}_wireNotesToggle(){let n=this._config||v,{view:e}=n;if(!this._container)return;this._container.querySelectorAll(".daily-notes").forEach(a=>{let d=a.querySelector(".notes-title"),l=a.querySelector(".notes-list");if(!d||!l)return;let f=d.cloneNode(!0);d.replaceWith(f);let u=r=>{a.classList.toggle("expanded",r),a.classList.toggle("collapsed",!r),f.setAttribute("aria-expanded",r?"true":"false")};u(e==="week");let s=r=>{if(r&&r.type==="keydown"){let o=r.key;if(o!=="Enter"&&o!==" "&&o!=="Spacebar")return;r.preventDefault()}let i=a.classList.contains("expanded");u(!i)};f.addEventListener("click",s),f.addEventListener("keydown",s)})}};window.customCards||(window.customCards=[]);window.customCards.push({type:_,name:"School Schedule Card",description:"Moderne, responsive Stundenplan-Karte f\xFCr Home Assistant."});customElements.get(_)||customElements.define(_,k);
